...
- Task pages should default to a single compact work queue. Use lightweight embedded task rows inside one shared list surface; keep the optional grid view secondary and avoid treating every task as a separate card by default.
...
- In dashboard project lists, show only the project service/type, project name, client name, and one progression bar with its current phase and percentage. Do not add duplicate health, active-state, package, or timeline tags to those rows.
...
- Keep all generic light surface utilities inside `FormSidebar` dark-compatible, including `bg-white`, `bg-(--color-surface)`, `bg-(--color-surface-alt)`, gray surface variants, and their matching text/border utilities. Fix these at the shared drawer stylesheet before adding page-specific overrides.
