...
export default function AddEventModal({ date, onAdd, onClose, initialEvent }: AddEventModalProps) {
  ...
  // Date range: 5 days before today to 20 days ahead
    ...
    >
      ...
      >
        ...
        <div className={`${config.iconBgClass} px-6 pt-6 pb-5 transition-colors duration-200`}>
          ...
          <h2 className={`text-xl font-bold ${config.textClass}`}>{initialEvent ? 'Edit Event' : 'New Event'}</h2>
          <p className={`text-sm font-medium ${config.textClass} opacity-60 mt-0.5`}>
              ...
              <label className="flex items-center gap-1.5 cursor-pointer select-none">
                <div
                  ...
                  className={`w-8 h-4.5 rounded-full relative transition-colors cursor-pointer
                    ${allDay ? 'bg-(--color-ink)' : 'bg-gray-200'}`}
                >
                ...
                return (
                  <button
                    ...
                    ${isActive
                        ...
                        : 'bg-(--color-surface) text-gray-600 hover:bg-gray-200'
                      }`}
                  >
                    <span className={`text-[9px] font-bold uppercase tracking-widest
                    ${isActive ? 'text-white/60' : isToday ? 'text-blue-500' : 'text-gray-400'}`}>
                      {format(d, 'EEE')}
                    </span>
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-center gap-1.5 py-3 px-1 rounded-2xl transition-all border
                      ${isActive
                        ? 'bg-(--color-ink) border-(--color-ink) shadow-sm'
                        : `${c.bgClass} border-transparent hover:border-gray-200`
                      }`}
                  >
                    <MaterialIcon
                      name={c.icon}
                    ...
                    <span className={`text-[9px] font-bold leading-none tracking-tight text-center
                      ${isActive ? 'text-white/80' : c.textClass}`}>
                      {c.shortLabel}
                    </span>
                  </button>
                );
