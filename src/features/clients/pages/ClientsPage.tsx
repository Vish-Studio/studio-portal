...
export default function Clients() {
  ...
  const onSubmit = async (data: ClientFormValues) => {
    ...
    try {
      ...
      showToast({
        status: 'error',
        title: editingClient ? FEEDBACK_MESSAGES.sidebar.clientUpdateToast : FEEDBACK_MESSAGES.sidebar.clientCreateToast,
        message,
      });
    }
  };
      ...
      >
        <form onSubmit={handleSubmit(onSubmit, onInvalidSubmit)} className="flex flex-col flex-1 min-h-0">
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
            <FormSidebarError
              title={editingClient ? FEEDBACK_MESSAGES.sidebar.clientUpdateFailed : FEEDBACK_MESSAGES.sidebar.clientCreateFailed}
              message={submitError}
            />
