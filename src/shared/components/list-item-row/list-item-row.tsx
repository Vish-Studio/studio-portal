...
export default function ListItemRow<T = unknown>({
  ...
  return (
    <div
      ...
      className={`list-item-row grid gap-3 rounded-[18px] border border-gray-200 bg-white p-4 text-left transition-colors hover:bg-gray-50 ${interactive ? 'cursor-pointer' : ''} ${gridClassName} lg:items-center ${className}`}
    >
      <div className="list-item-row-primary flex min-w-0 items-center gap-3">
        ...
        <div className="list-item-row-copy min-w-0">
          <div className="list-item-row-title-wrap flex min-w-0 flex-wrap items-center gap-2">
            ...
            <div className="list-item-row-mobile-status lg:hidden">{status}</div>
          </div>
          {subtitle && <div className={`list-item-row-subtitle mt-1 ${subtitleClassName}`}>{subtitle}</div>}
        </div>
      </div>

      <div className="list-item-row-secondary min-w-0">{secondary}</div>
      <div className="list-item-row-tertiary min-w-0">{tertiary}</div>
      <div className="list-item-row-status hidden justify-end lg:flex">{status}</div>
