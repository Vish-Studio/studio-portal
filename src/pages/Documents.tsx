...
export default function Documents() {
  ...
  const { documents, setDocuments, signDocument } = useDocumentsStore();
    ...
    const fetchDocs = async () => {
      try {
        const q = appUser.role === 'admin'
          ...
          : query(collection(db, 'documents'), where('clientId', 'in', [user.uid, 'global']));
        const snap = await getDocs(q);
        const list: StudioDocument[] = snap.docs
    ...
    try {
      if (!isDemo) {
        await updateDoc(doc(db, 'documents', signingDoc.id), {
          isSigned: true,
          signatureData: { name: signatureName, uid: user.uid, timestamp: new Date().toISOString() },
        });
      }
      signDocument(signingDoc.id, signatureName, user.uid);
      setSigningDoc(null);
      setSignatureName('');
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {documents.map(document => {
              ...
              return (
                <div
                  key={document.id}
                  className="bg-white border border-gray-200 rounded-[28px] p-6 hover:shadow-lg transition-all group flex flex-col justify-between h-full hover:border-gray-300 relative"
                >
                  {pendingSignature && (
                  ...
                  <div>
                    ...
                    <h3 className="text-lg font-extrabold text-gray-900 mb-2 leading-tight group-hover:text-blue-600 transition-colors">
                      {document.title}
                    </h3>
                  ...
                  <div className="mt-auto flex flex-col gap-2">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center justify-center w-full px-6 py-3.5 bg-gray-50 text-gray-900 border border-gray-200 font-bold text-xs rounded-[16px] hover:bg-gray-100 transition-colors shadow-sm"
                    >
                      Open Document
                    ...
                    {pendingSignature && appUser?.role === 'client' && (
                      <button
                        onClick={() => setSigningDoc(document)}
                        className="flex items-center justify-center w-full px-6 py-3.5 bg-black text-white font-extrabold text-xs uppercase tracking-widest rounded-[16px] hover:bg-gray-800 transition-colors shadow-sm"
                      >
                        Sign Now
                      </button>
                    )}
