...
import { Briefcase, CheckCircle, Layers, Trash2, Pencil } from 'lucide-react';
...
import { SERVICE_META, ALL_STAGES, getProjectAccent } from '@/src/data/projects';
...
// ─── Inline card (grid) — mirrors ProjectCard but without admin navigation ─────
    ...
    <div className="project-card bg-white border border-gray-200 rounded-[16px] hover:border-gray-300 transition-all duration-150 flex flex-col">
      <div className="px-4 pt-4 pb-3">
        ...
        <p className="text-[11px] text-gray-400 truncate">
          {accent.label} · {project.timeline || 'TBD'}
        </p>

        <div className="mt-3.5">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-[10px] text-gray-400">
              {done}/{ALL_STAGES.length} phases
...
// ─── Inline mini-row (list) — mirrors ProjectCardMini ─────────────────────────
...
const UserProjectCardMini = ({
  ...
  return (
    <div className="project-card-mini relative bg-white border border-gray-200 rounded-[12px] px-3 py-3 hover:bg-gray-50 hover:border-gray-300 transition-all duration-150 md:px-4">
      <div className="grid items-center gap-3 md:grid-cols-[minmax(200px,1fr)_120px_90px_110px_120px_32px]">
        {/* Project */}
...
const UserProjects = () => {
  ...
  const openEdit = (project: ClientProject) => {
    ...
    reset({
      name: project.name, service: project.service,
      package: project.package ?? '', status: project.status, timeline: project.timeline,
    });
    setSidebarOpen(true);
  };
  ...
  return (
    <UserLayout title="My Projects">
      <div className="flex flex-col gap-5 pb-10">
        ...
        {filtered.length === 0 ? (
          <div className="bg-white border border-gray-100 rounded-[18px] py-16 flex flex-col items-center gap-3 text-center">
            ...
            <button
              onClick={openAdd}
              className="mt-1 text-xs font-semibold text-gray-400 hover:text-gray-700 underline transition-colors"
            >
          ...
          <div className="flex flex-col gap-2">
            <div className="hidden grid-cols-[minmax(200px,1fr)_120px_90px_110px_120px_32px] items-center gap-3 px-4 text-[11px] font-semibold uppercase tracking-wide text-gray-400 md:grid">
              <span>Project</span>
      ...
      <FormSidebar
        ...
        description={editingProject ? `Editing ${editingProject.name}` : `Adding a project for ${currentClient.displayName}`}
        width="md"
      >
